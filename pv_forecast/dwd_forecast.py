...
#from wetterdienst.provider.dwd.forecast.metadata.dates import DwdForecastDate
...
class DWD_Forecast:
    ...
    def retrieve_data(self):
        ...
        respone = self.request.values.all()
    ...
    def reshape_data(self, raw_data):
        ...
        """
        my_raw_data = raw_data.to_pandas()
        # First, create a table from plain DWD data using DATE as index
        reshaped_data = pd.pivot_table(my_raw_data, values="value", index=["date"], columns="parameter", aggfunc='first')
        ...
        #reshaped_data.columns = reshaped_data.columns.add_categories(["TEMPERATURE_AIR_200DEGC", "DEW_POINT_DEGC", "RAD_WH", "wind_spd"])
        #reshaped_data.columns = reshaped_data.columns.add_categories(["TEMPERATURE_AIR_200DEGC", "DEW_POINT_DEGC", "RAD_WH", "wind_spd"])
        # For pvlib, temperatures need to be available in degC:
        reshaped_data["TEMPERATURE_AIR_200DEGC"] = reshaped_data["temperature_air_mean_200"].apply(lambda x: x - 273.15)
        reshaped_data["DEW_POINT_DEGC"] = reshaped_data["temperature_dew_point_mean_200"].apply(lambda x: x - 273.15)
